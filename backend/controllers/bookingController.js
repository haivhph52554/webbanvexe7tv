...
exports.checkout = async (req, res) => {
  ...
  try {
    ...
    const runFlow = async (maybeSession) => {
      ...
      // Nếu là Banking, MoMo hoặc VNPay -> Pending. COD -> Paid (hoặc pending tùy bạn, ở đây để paid cho đơn giản)
      const initialStatus = (paymentMethod === 'banking' || paymentMethod === 'momo' || paymentMethod === 'vnpay') ? 'pending' : 'paid';
