...
import { ArrowLeft, Bus, MapPin, Clock, Calendar, Download, Home, User, Phone, Mail, FileText, AlertCircle, CreditCard } from 'lucide-react';
  ...
  useEffect(() => {
    ...
    const fetchPaymentInfo = async () => {
      try {
        ...
        const res = await fetch(`${API_BASE}/api/bookings/${ticket.id}`);
        if (res.ok) {
          const booking = await res.json();
          if (booking.payment) {
            // Fetch payment details
            const paymentRes = await fetch(`${API_BASE}/api/payments/${booking.payment}`);
            if (paymentRes.ok) {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-xl shadow-md p-6">
              ...
              {/* Hiển thị QR Code nếu vé đang pending và phương thức là banking/momo */}
              {ticket.status === 'pending' && (ticket.paymentMethod === 'banking' || ticket.paymentMethod === 'momo') && (
                <div className="bg-yellow-50 border-2 border-yellow-400 rounded-xl p-6 mb-6 text-center">
                  ...
                  <div className="bg-white p-4 inline-block rounded-lg shadow-sm border mb-4">
                    ...
                    <img 
                      src={`https://img.vietqr.io/image/BIDV-8851715585-compact.jpg?amount=${ticket.totalAmount}&addInfo=VEXE ${ticket.bookingId}`} 
                  ...
                  <div className="text-sm text-gray-800 space-y-2">
                    ...
                    <p className="font-mono font-bold text-red-600 text-lg bg-white inline-block px-3 py-2 rounded border border-red-200">
                      VEXE {ticket.bookingId}
                    </p>
                    <p className="mt-3 text-red-600 text-xs italic">
              ...
              <div className="space-y-3 mb-4">
                <div className="flex justify-between">
                  ...
                  <span className="font-medium font-mono">{ticket.bookingId}</span>
                </div>
                  ...
                  <>
                    {paymentInfo.paymentId && (
                      <div className="flex justify-between">
                        ...
                        <span className="font-medium font-mono text-blue-600">{paymentInfo.paymentId}</span>
                      </div>
                    )}
                    {paymentInfo.transaction_no && (
                      <div className="flex justify-between">
                        <span className="text-gray-600">Mã giao dịch:</span>
                        <span className="font-medium font-mono text-green-600">{paymentInfo.transaction_no}</span>
                      </div>
                    )}
                    {paymentInfo.bank_code && (
                      <div className="flex justify-between">
                        <span className="text-gray-600">Ngân hàng:</span>
                        <span className="font-medium">{paymentInfo.bank_code}</span>
                      </div>
                    )}
                    {paymentInfo.paid_at && (
                      <div className="flex justify-between">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">{getPaymentMethodText(ticket.paymentMethod || paymentInfo?.method)}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-600">Số ghế:</span>
                  <span className="font-medium">{ticket.seats.length} ghế ({ticket.seats.join(', ')})</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-600">Giá vé/ghế:</span>
                  <span className="font-medium">{parseInt(ticket.route.price) / ticket.seats.length}₫</span>
                </div>
                <div className="flex justify-between">
