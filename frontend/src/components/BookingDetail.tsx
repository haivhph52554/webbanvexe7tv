...
const BookingDetail: React.FC = () => {
  ...
  // Validate stops (hide stop with order === 2 and validate against visible stops)
  const displayedStops = React.useMemo(() => (tripDetail?.stops || []).filter(s => s.order !== 2), [tripDetail?.stops]);

  useEffect(() => {
    if (!displayedStops || displayedStops.length === 0) return;
    const hasPickup = selectedPickupId ? displayedStops.some(s => s._id === selectedPickupId) : true;
    const hasDropoff = selectedDropoffId ? displayedStops.some(s => s._id === selectedDropoffId) : true;
    if (!hasPickup) setSelectedPickupId(null);
    if (!hasDropoff) setSelectedDropoffId(null);
    if (selectedPickupId && selectedDropoffId) {
      const pu = displayedStops.find(s => s._id === selectedPickupId);
      const dr = displayedStops.find(s => s._id === selectedDropoffId);
      if (pu && dr && pu.order >= dr.order) {
        setSelectedDropoffId(null);
      }
    }
  }, [displayedStops, selectedPickupId, selectedDropoffId]);
  ...
  const handleBooking = () => {
    ...
    if (!selectedPickupId || !selectedDropoffId) { alert('Vui lòng chọn điểm đón và điểm trả trước khi thanh toán'); return; }
    ...
    navigate('/payment', { state: {
      ...
      stops: { pickupId: selectedPickupId, dropoffId: selectedDropoffId, pickupName: displayedStops.find(s=>s._id===selectedPickupId)?.stop_name, dropoffName: displayedStops.find(s=>s._id===selectedDropoffId)?.stop_name }
    } });
  };
            ...
            <div className="lg:col-span-1">
              <div className="bg-white rounded-xl shadow-md p-6 sticky top-8">
                ...
                <button
                  ...
                  className={`w-full py-3 px-4 rounded-lg font-medium text-lg transition-colors ${(!selectedPickupId || !selectedDropoffId) ? 'bg-gray-400 text-white cursor-not-allowed' : 'bg-blue-600 text-white hover:bg-blue-700'}`}
                >
                  Tiếp tục thanh toán
                </button>
                {(!selectedPickupId || !selectedDropoffId) && <p className="text-sm text-red-600 mt-2">Vui lòng chọn điểm đón và điểm trả trước khi thanh toán.</p>}
              </div>
            </div>
          </div>
        )}
